...
private struct MVDDocumentBrowser: View {
    ...
    @State private var cmmPortalStatus = "CMM link ready. Sign in and acknowledge any portal notice."
    ...
    /// CMM opens the exact trained URL in the embedded browser, following the
    /// same direct-link handoff Android uses (ACTION_VIEW) rather than trying
    /// to reconstruct a publication from Pinpoint's library tree.
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 8) {
                ...
                ZStack {
                    if isCMM {
                        MVDCMMPortalView(target: target, status: $cmmPortalStatus)
                    } else {
                    MVDDocumentWebView(
...
private struct MVDCMMPortalView: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Button {
                status = "Opening the trained CMM link…"
                // A fresh top-level request resumes the exact URL in the same
                // WKWebView; its original #page=N fragment is left untouched.
            ...
            .buttonStyle(.borderedProminent)
            .tint(.blue)
            .accessibilityHint("Opens the exact trained CMM link and its saved page in this browser session")
        }
    }
}
...
private struct MVDCMMPortalWebView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        ...
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            onStatus("Loading the trained CMM link…")
        }

        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            onStatus("CMM page loaded. Complete sign-in or acknowledge any portal notice; tap OPEN DOC to retry the trained page.")
        }
